...
void define_opt(Cmd *cmd, Define_Opts opts) {
    ...
    if (opts.space_decl) {
        CFLAGS(cmd, "-DSPACEDECL=");
    }
    if (opts.space_def) {
        CFLAGS(cmd, "-DSPACEDEF=inline");
    }

    if (opts.space_alloc_method_virtual_alloc) {
    ...
    if (0) {
        ...
        CFLAGS(cmd, "-DSPACE_MEMORY_LAYOUT_METHOD_DEFAULT=SPACE_MEMORY_DOUBLE_LINKED_LIST");
    } else if (opts.space_memory_layout_method_da) {
        CFLAGS(cmd, "-DSPACE_MEMORY_LAYOUT_METHOD_DEFAULT=SPACE_MEMORY_DYNAMIC_ARRAY");
    }
...
void tests_opt(Cmd *cmd, Tests_Opts opts) {
    ...
    if (0) {
        ...
        define(cmd, .include_raylib = true, .tkbc_server = true, .print_operation_and_description = true);
...
void usage_opt(Usage_Opts opts) {
    ...
    if (opts.test || opts.all) {
        fprintf(stderr, "       <%s> <%s> [%s|%s]\n", opts.prog_name, FLAG_TEST, FLAG_TEST_VERBOSE, FLAG_TEST_SHORT);
    }
