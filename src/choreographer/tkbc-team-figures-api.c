...
bool tkbc_script_team_ball(Env *env, Kite_Ids kite_index_array,
  ...
  for (size_t i = 0; i < kite_index_array.count; ++i) {
    ...
    {
      Frame *frame = KITE_MOVE(ID(kite_index_array.elements[i]), place.x,
                               place.y, move_duration);
      if (frame == NULL)
        return false;
    ...
    {
      Frame *frame = KITE_ROTATION(ID(kite_index_array.elements[i]), deg_angle,
                                   rotation_duration);
      if (frame == NULL)
        return false;
...
bool tkbc_script_team_mountain(Env *env, Kite_Ids kite_index_array,
  ...
  for (size_t i = 0, column = 0; column < columns; ++i, ++column) {
    ...
    {
      Frame *frame = KITE_MOVE(ID(kite_index_array.elements[i]),
                               anchor.x + x_space * column,
                               anchor.y + y_space * row, move_duration);
      if (frame == NULL)
        return false;
...
bool tkbc_script_team_arc(Env *env, Kite_Ids kite_index_array, Vector2 position,
  ...
  for (size_t i = 0, column = 0; column < columns; ++i, ++column) {
    ...
    {
      Frame *frame = KITE_ROTATION(ID(kite_index_array.elements[i]), angle,
                                   rotation_duration);
      if (frame == NULL)
        return false;
...
bool tkbc_script_team_split_box_up(Env *env, Kite_Ids kite_index_array,
  ...
  case ODD: {
    for (int i = kite_index_array.count - 1; i >= 0;) {
      frame = KITE_ROTATION_ADD(ID(kite_index_array.elements[i--]), angle,
                                rotation_duration);
      if (frame == NULL)
        return false;
    ...
    for (int i = kite_index_array.count - 1; i >= 0;) {
      frame = KITE_MOVE_ADD(ID(kite_index_array.elements[i--]), 0, -box_size,
                            move_duration);
      if (frame == NULL)
        return false;
    ...
    for (int i = kite_index_array.count - 1; i >= 0;) {
      frame = KITE_MOVE_ADD(ID(kite_index_array.elements[i--]), -box_size, 0,
                            move_duration);
      if (frame == NULL)
        return false;
