 ...
 * otherwise false.
 */
bool tkbc_script_team_roll_two_diffrent_angle(Env *env, Kite_Ids kite_index_array, float radius, size_t begin_angle_1,
    ...
    for (size_t deg = 0; deg < first; ++deg) {
        ...
        {
            frame = KITE_MOVE_ADD(ID(kite_index_array.elements[0]), position.x, position.y, duration_1);
            if (frame == NULL) return false;
            space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
        }
        {
            frame = KITE_ROTATION_ADD(ID(kite_index_array.elements[0]), 1, duration_1);
            if (frame == NULL) return false;
            space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
        }
        ...
        {
            frame = KITE_MOVE_ADD(ID(kite_index_array.elements[1]), position.x, position.y, duration_2);
            if (frame == NULL) return false;
            space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
        }
        {
            frame = KITE_ROTATION_ADD(ID(kite_index_array.elements[1]), 1, duration_2);
            if (frame == NULL) return false;
            space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
        }
 ...
 * otherwise false.
 */
bool tkbc_script_team_roll_split_up(Env *env, Kite_Ids kite_index_array, ODD_EVEN odd_even, float radius,
                                    size_t begin_angle, size_t end_angle, float move_duration) {
    Frame *frame = NULL;
    ...
    for (size_t deg = begin_angle; deg < end_angle; ++deg) {
        ...
        for (size_t i = 0; i < kite_index_array.count; ++i) {
            if (i % 2 == odd_even) {
                ...
                {
                    frame = KITE_MOVE_ADD(ID(kite_index_array.elements[i]), position.x, position.y, duration);
                    if (frame == NULL) return false;
                    space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
                }
                {
                    frame = KITE_ROTATION_ADD(ID(kite_index_array.elements[i]), 1, duration);
                    if (frame == NULL) return false;
                    space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
                }
            } else {
 ...
 * otherwise false.
 */
bool tkbc_script_team_roll_split_down(Env *env, Kite_Ids kite_index_array, ODD_EVEN odd_even, float radius,
                                      size_t begin_angle, size_t end_angle, float move_duration) {
    Frame *frame = NULL;
    float duration = move_duration / (end_angle - begin_angle);
    int sign;
    ...
    for (size_t deg = begin_angle; deg < end_angle; ++deg) {
        ...
        for (size_t i = 0; i < kite_index_array.count; ++i) {
            ...
            {
                frame = KITE_ROTATION_ADD(ID(kite_index_array.elements[i]), sign, duration);
                if (frame == NULL) return false;
                space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
            }
        }
 ...
 * otherwise false.
 */
bool tkbc_script_team_roll_up_anti_clockwise(Env *env, Kite_Ids kite_index_array, float radius, size_t begin_angle,
 ...
 * otherwise false.
 */
bool tkbc_script_team_roll_up_clockwise(Env *env, Kite_Ids kite_index_array, float radius, size_t begin_angle,
                                        size_t end_angle, float move_duration) {
    Frame *frame = NULL;
    float duration = move_duration / (end_angle - begin_angle);
    for (size_t deg = begin_angle; deg < end_angle; ++deg) {
 ...
 * otherwise false.
 */
bool tkbc_script_team_roll_down_anti_clockwise(Env *env, Kite_Ids kite_index_array, float radius, size_t begin_angle,
                                               size_t end_angle, float move_duration) {
    Frame *frame = NULL;
    float duration = move_duration / (end_angle - begin_angle);
    for (size_t deg = begin_angle; deg < end_angle; ++deg) {
 ...
 * otherwise false.
 */
bool tkbc_script_team_roll_down_clockwise(Env *env, Kite_Ids kite_index_array, float radius, size_t begin_angle,
                                          size_t end_angle, float move_duration) {
    Frame *frame = NULL;
    float duration = move_duration / (end_angle - begin_angle);
    for (size_t deg = begin_angle; deg < end_angle; ++deg) {
 ...
 * otherwise false.
 */
bool tkbc_script_team_ball(Env *env, Kite_Ids kite_index_array, Vector2 position, Vector2 offset, float radius,
    ...
    for (size_t i = 0; i < kite_index_array.count; ++i) {
        ...
        float deg_angle = (180 - (180 - (deg_base_rotation + 90))) + deg_base_rotation;
        deg_base_rotation += segment_size;
        {
            frame = KITE_MOVE(ID(kite_index_array.elements[i]), place.x, place.y, move_duration);
            if (frame == NULL) return false;
            space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
        }
        {
            frame = KITE_ROTATION(ID(kite_index_array.elements[i]), deg_angle, rotation_duration);
            if (frame == NULL) return false;
            space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
        }
    }
 ...
 * otherwise false.
 */
bool tkbc_script_team_mountain(Env *env, Kite_Ids kite_index_array, Vector2 position, Vector2 offset, float v_padding,
                               float h_padding, float move_duration, float rotation_duration) {
    ...
    size_t rows = isodd ? kite_index_array.count / 2 + 1 : kite_index_array.count / 2;
    ...
    for (size_t i = 0, column = 0; column < columns; ++i, ++column) {
        ...
        {
            frame = KITE_MOVE(ID(kite_index_array.elements[i]), anchor.x + x_space * column, anchor.y + y_space * row,
                              move_duration);
            if (frame == NULL) return false;
            space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
        }
        {
            frame = KITE_ROTATION(ID(kite_index_array.elements[i]), 0, rotation_duration);
            if (frame == NULL) return false;
            space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
        }
 ...
 * otherwise false.
 */
bool tkbc_script_team_valley(Env *env, Kite_Ids kite_index_array, Vector2 position, Vector2 offset, float v_padding,
                             float h_padding, float move_duration, float rotation_duration) {
 ...
 * otherwise false.
 */
bool tkbc_script_team_arc(Env *env, Kite_Ids kite_index_array, Vector2 position, Vector2 offset, float v_padding,
                          float h_padding, float angle, float move_duration, float rotation_duration) {
    ...
    for (size_t i = 0, column = 0; column < columns; ++i, ++column) {
        ...
        {
            frame = KITE_ROTATION(ID(kite_index_array.elements[i]), angle, rotation_duration);
            if (frame == NULL) return false;
 ...
 * otherwise false.
 */
bool tkbc_script_team_mouth(Env *env, Kite_Ids kite_index_array, Vector2 position, Vector2 offset, float v_padding,
                            float h_padding, float angle, float move_duration, float rotation_duration) {
    angle = -angle;
    float start_angle = angle;
 ...
 * otherwise false.
 */
bool tkbc_script_team_line(Env *env, Kite_Ids kite_index_array, Vector2 position, Vector2 offset, float h_padding,
                           float move_duration) {

    return tkbc_script_team_grid(env, kite_index_array, position, offset, 0, h_padding, 1, kite_index_array.count,
                                 move_duration);
}
 ...
 * otherwise false.
 */
bool tkbc_script_team_grid(Env *env, Kite_Ids kite_index_array, Vector2 position, Vector2 offset, float v_padding,
                           float h_padding, size_t rows, size_t columns, float move_duration) {
 ...
 * should take in seconds.
 */
void tkbc_script_team_box(Env *env, Kite_Ids kite_index_array, DIRECTION direction, float angle, float box_size,
                          float move_duration, float rotation_duration) {
    ...
    switch (direction) {
    case LEFT: SET(KITE_MOVE_ADD(kite_index_array, -box_size, 0, move_duration)); break;
    case RIGHT: SET(KITE_MOVE_ADD(kite_index_array, box_size, 0, move_duration)); break;
 ...
 * should take in seconds.
 */
void tkbc_script_team_box_left(Env *env, Kite_Ids kite_index_array, float box_size, float move_duration,
                               float rotation_duration) {
    tkbc_script_team_box(env, kite_index_array, LEFT, 90, box_size, move_duration, rotation_duration);
}
 ...
 * should take in seconds.
 */
void tkbc_script_team_box_right(Env *env, Kite_Ids kite_index_array, float box_size, float move_duration,
                                float rotation_duration) {
    tkbc_script_team_box(env, kite_index_array, RIGHT, 90, box_size, move_duration, rotation_duration);
}
 ...
 * otherwise false.
 */
bool tkbc_script_team_split_box_up(Env *env, Kite_Ids kite_index_array, ODD_EVEN odd_even, float box_size,
                                   float move_duration, float rotation_duration) {
    tkbc_reset_frames_internal_data(&env->scratch_buf_frames);
    Frame *frame = NULL;
    ...
    case ODD: {
        for (int i = kite_index_array.count - 1; i >= 0;) {
            frame = KITE_ROTATION_ADD(ID(kite_index_array.elements[i--]), angle, rotation_duration);
            if (frame == NULL) return false;
            space_dap(&env->scratch_buf_script.space, &env->scratch_buf_frames, *frame);
        ...
        for (int i = kite_index_array.count - 1; i >= 0;) {
            frame = KITE_MOVE_ADD(ID(kite_index_array.elements[i--]), 0, -box_size, move_duration);
            if (frame == NULL) return false;
        ...
        for (int i = kite_index_array.count - 1; i >= 0;) {
            frame = KITE_MOVE_ADD(ID(kite_index_array.elements[i--]), -box_size, 0, move_duration);
            if (frame == NULL) return false;
 ...
 * should take in seconds.
 */
void tkbc_script_team_diamond(Env *env, Kite_Ids kite_index_array, DIRECTION direction, float angle, float box_size,
                              float move_duration, float rotation_duration) {
    ...
    SET(KITE_MOVE_ADD(kite_index_array, sign * box_size / sqrt2, -box_size / sqrt2, move_duration));
 ...
 * should take in seconds.
 */
void tkbc_script_team_diamond_left(Env *env, Kite_Ids kite_index_array, float box_size, float move_duration,
                                   float rotation_duration) {
    tkbc_script_team_diamond(env, kite_index_array, LEFT, 90, box_size, move_duration, rotation_duration);
}
 ...
 * should take in seconds.
 */
void tkbc_script_team_diamond_right(Env *env, Kite_Ids kite_index_array, float box_size, float move_duration,
                                    float rotation_duration) {
    tkbc_script_team_diamond(env, kite_index_array, RIGHT, 90, box_size, move_duration, rotation_duration);
}
