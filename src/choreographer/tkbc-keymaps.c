...
bool tkbc_save_keymaps_to_file(Key_Maps keymaps, const char *filename) {
    ...
    for (size_t i = 0; i < keymaps.count; ++i) {
        fprintf(file, "%d %d %d %d\n", keymaps.elements[i].hash, keymaps.elements[i].mod_key,
                keymaps.elements[i].selection_key, keymaps.elements[i].key);
    }
...
void tkbc_setup_keymaps_strs(Key_Maps *keymaps) {
    for (size_t i = 0; i < keymaps->count; ++i) {
        keymaps->elements[i].mod_key_str = tkbc_key_to_str(keymaps->elements[i].mod_key);
        keymaps->elements[i].selection_key_str = tkbc_key_to_str(keymaps->elements[i].selection_key);
...
bool tkbc_check_key(int key, Key_Mode mode) {
    ...
    switch (mode) {
    case MODE_DOWN: check = IsKeyDown(key); break;
    case MODE_PRESSED: check = IsKeyPressed(key); break;
    case MODE_UP: check = IsKeyUp(key); break;
    case MODE_RELEASED: check = IsKeyReleased(key); break;
    case MODE_NULL:
...
bool tkbc_check_keymap(Key_Map keymap, Key_Map_Check_Config cfg, int kso) {
    ...
    if (keymap.selection_key && (kso & SELECTION_KEY) && (cfg.selection_key != MODE_NULL)) {
    ...
    if (has_key && has_selection) {
        base = cfg.is_or ? (key_check || selection_check) : (key_check && selection_check);
    } else if (has_key) {
        base = key_check;
