...
int tkbc_write_file_mode(const char *filename, const void *buffer, size_t size, const char *mode);
...
bool tkbc__make_dir_if_not_existis_internal(const char *path, bool report_err, bool report_exists);
bool tkbc_make_dir_recursive_if_not_existis(const char *path);
bool tkbc__make_dir_recursive_if_not_existis_internal(const char *path, bool report_err);
...
long tkbc_read_entire_file(const char *filename, Content *content) {
    ...
    if (content->count + size > content->capacity) {
        ...
        content->elements = realloc(content->elements, sizeof(*content->elements) * size_pow2);
    ...
    size_t read_count = fread(content->elements + content->count, sizeof(*content->elements), size, file);
...
#define MOUSE_BUTTON_COUNT 7
    ...
    static_assert(MOUSE_BUTTON_COUNT == MOUSE_BUTTON_BACK + 1, "More mouse buttons need to be handled.");
    ...
    if (IsMouseButtonReleased(mouse_button)) {
        if (state[mouse_button].waiting) {
            ...
            if (delta >= 0 && delta <= TKBC_MAX_DOUBLECLICK_MS) result = true;
        }
        if (!result) {
            state[mouse_button].last_release_ms = now_ms;
            state[mouse_button].waiting = true;
        }
    } else if (state[mouse_button].waiting && now_ms - state[mouse_button].last_release_ms > TKBC_MAX_DOUBLECLICK_MS) {
        state[mouse_button].waiting = false;
    }
...
static int bytes_per_pixel_from_format(PixelFormat format) {
    ...
    case PIXELFORMAT_COMPRESSED_ASTC_8x8_RGBA: return 0;
...
char tkbc_get_file_type(const char *file_path) {
    ...
    if (attributes == INVALID_FILE_ATTRIBUTES) {
        tkbc_fprintf(stderr, "ERROR", "Failed to get file information for %s: %lu\n", file_path, GetLastError());
    ...
    if (attributes & FILE_ATTRIBUTE_NORMAL || !(attributes & (FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_DEVICE))) {
        return TKBC_DT_REG;
    }
...
bool tkbc_remove(const char *path) {
    ...
    case TKBC_DT_REG:
        ...
        break;
    default: assert("tkbc_remove_dir_recursive: Unsupported file type"); ok = false;
    }
