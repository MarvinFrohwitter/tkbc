...
typedef struct {
    ...
    bool is_upscaled;                     // True for blocks generated by upscaling
                                          // (per-tick slices), false for authored
                                          // original blocks.
...
typedef struct {
    Frames *elements;  // The dynamic array collection for all combined frames as a
                       // script. After upscaling these are the per-tick slices
                       // used for playback and scrubbing.
    size_t count;      // The amount of elements in the array.
    size_t capacity;   // The complete allocated space for the array represented as
                       // the number of collection elements of the array type.
    Frames *original_elements;  // The pre-upscale blocks in stripped form: actions,
                                // durations and kite ids only, without positions
                                // (re-derived at load by patch + bake, never
                                // exported/sent). NULL when the script was never
                                // expanded. Disk export and network send use
                                // these so the original non-upscaled script is
                                // preserved at minimal footprint.
    size_t original_count;      // The amount of elements in original_elements.
    size_t original_capacity;   // The allocated space of original_elements.
...
typedef struct {
    ...
    size_t default_scripts_seq;     // Build-order sequence for deterministic default uuids.
                                    // Reset to 0 before each default build; incremented per
                                    // default script so ids stay stable even if scripts were
