...
typedef struct {
    ...
    Color cross_color;         // The background color of the cross that normally closes
                               // the pop-up.
    Rectangle option1;         // The bounding box that represents the first selection option.
    Color option1_color;       // The background color of the first option bounding box.
    const char *option1_text;  // The text that should be displayed in the first option.

    const char *text;  // The text that is displayed in the base container of the pop-up.
